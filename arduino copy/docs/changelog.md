...
## [2024-12-19] - Доработка логики хоминга с homeBackoff

### Изменено
- **Улучшена логика хоминга в функции stepperHome()**:
  - Если датчик уже сработал: отъезжаем до его отключения + еще homeBackoff шагов
  - Если датчик не сработал: находим концевик + отъезжаем на homeBackoff шагов
  - Нулевая позиция устанавливается ПОСЛЕ отъезда на homeBackoff расстояние
  - Добавлено детальное логирование каждого этапа хоминга

### Исправлено
- **КРИТИЧЕСКОЕ: Устранены зависания программы во время хоминга**:
  - Добавлены таймауты безопасности для всех циклов ожидания
  - Защита от бесконечных циклов при поиске концевика (30 сек или maxSteps)
  - Таймауты ожидания готовности планировщика (3-5 сек)
  - Обработка ошибок при недостижимости концевика
  - Принудительная очистка планировщика с таймаутом
- **ОШИБКА КОМПИЛЯЦИИ: Исправлено повторное объявление переменной startPos**:
  - Переименованы переменные: `startPosTriggered`, `startPosSearch`, `startPosFound`
  - Устранен конфликт имен в разных блоках кода
  - Успешная компиляция: 23360 байт (9.2% Flash), 4893 байт (59.7% RAM)

### Техническая реализация
- **Двухэтапный отъезд при уже сработавшем датчике**:
  1. Отъезд до отключения датчика (таймаут 10 сек)
  2. Дополнительный отъезд на homeBackoff шагов (таймаут 10 сек)
- **Стандартный хоминг с отъездом**:
  1. Поиск концевика (таймаут 30 сек или maxSteps)
  2. Отъезд на homeBackoff шагов (таймаут 10 сек)
- **Установка нуля**: происходит в финальной позиции после всех отъездов
- **Защита от зависаний**: таймауты на каждом этапе с обработкой ошибок
...
- **Обработка ошибок**: продолжение работы при недостижимости концевика
- **Паузы между моторами**: 100 мс для стабилизации

### Параметры homeBackoff из конфигурации
- Multi(X): 200 шагов
- Multizone(Y): 200 шагов  
- RRight(Z): 1000 шагов
- E0: 200 шагов

### Логирование процесса
```
Multi(X) - endstop already triggered, moving away to backoff distance
Multi(X) - moved away from endstop, now moving to backoff distance
Multi(X) - moved to backoff distance (200 steps from endstop)
Multi(X) position set to zero - homing completed
```

### Сообщения об ошибках
```
ERROR: Could not move away from endstop
ERROR: Endstop not found within limits  
WARNING: Planner cleanup timeout - system may be unstable
```

### Результат компиляции
- **Размер прошивки**: 23360 байт (9.2% Flash)
- **Использование RAM**: 4893 байт (59.7%)  
- **Статус**: ✅ Успешная компиляция без ошибок
...
## [2024-12-19] - НОВАЯ ФУНКЦИОНАЛЬНОСТЬ: Индивидуальное управление питанием двигателей
### Добавлено
- ✅ **ИНДИВИДУАЛЬНОЕ УПРАВЛЕНИЕ ПИТАНИЕМ**: Настройка `powerAlwaysOn[NUM_MOTORS]` для каждого двигателя
- ✅ Двигатели с постоянным питанием (Multi, Multizone, RRight) остаются включенными всегда
- ✅ Двигатели с временным питанием (E0, E1) выключаются после движения
- ✅ Автоматическая инициализация питания при старте системы
- ✅ Функции `enableMotor()` и `disableMotor()` для индивидуального управления
- ✅ Обновленная команда `status` с информацией о настройках питания
- ✅ Подробная диагностика состояния питания каждого двигателя

### Изменено
**Настройки питания двигателей:**
```cpp
// НОВАЯ НАСТРОЙКА: Управление питанием двигателей
const bool powerAlwaysOn[NUM_MOTORS] = {true, true, true, false, false};
...
```cpp
void enableMotor(int motorIndex);   // Включение конкретного двигателя
void disableMotor(int motorIndex);  // Отключение с учетом настройки powerAlwaysOn
```

**Логика инициализации:**
- При старте системы двигатели с `powerAlwaysOn[i] = true` включаются автоматически
- Двигатели с `powerAlwaysOn[i] = false` остаются выключенными до первого движения

### Техническая информация
- **Размер прошивки**: 23296 байт (9.2% от 253952 байт)
- **Использование RAM**: 4909 байт (59.9% от 8192 байт)
...
## [2024-12-19] - Упрощение функции хоминга

### Изменено
- Переписана функция stepperHome() по образцу из примера GyverPlanner
- Убрана переусложненная логика с таймаутами и проверками готовности планировщика
- Упрощена логика хоминга: setSpeed -> while цикл с tick() -> brake() -> reset()
- Убраны сложные алгоритмы отъезда и позиционирования

### Техническое обоснование
- Исходная функция была переусложнена и не соответствовала стилю библиотеки GyverPlanner
- Новая реализация проще, надежнее и следует оригинальному примеру из документации
- Используется brake() для остановки и reset() для сброса координат в ноль

## [2024-12-19] - Исправление проблемы с занятым планировщиком и упрощение clampHome

### Исправлено
- Исправлена критическая проблема: планировщик всегда занят даже когда моторы не движутся
- Заменен planner.stop() на planner.reset() для корректной очистки планировщика
- Упрощена функция clampHome() по образцу stepperHome()
- Убрана переусложненная логика координированного движения в clampHome

### Изменено
- В moveMotorsToPosition(): заменен planner.stop() на planner.reset()
- В clampHome(): применена простая логика setSpeed -> while + tick() -> brake()
- Убраны delay() и сложные таймауты из clampHome
- Координированный хоминг E0+E1 упрощен до базовой логики

### Техническое обоснование
- planner.stop() останавливает движение но оставляет планировщик в состоянии "занят"
- planner.reset() полностью очищает планировщик и делает его готовым для новых команд
- Упрощенная логика clampHome следует тому же принципу что и stepperHome

## [2024-12-19] - Полная актуализация README

### Изменено
- Полностью переписан README.md с актуальной информацией о GyverPlanner
- Обновлена информация о типах датчиков (E0, E1 теперь PNP)
- Детализированы все команды с примерами использования
- Добавлена информация о координированном хоминге E0+E1 (clamph)
- Обновлена архитектурная документация системы
- Добавлены примеры кода и диагностики

### Добавлено
- Подробное описание команд: sm, sh, clamph, pon/poff, status, test
- Объяснение различий между индивидуальным и координированным хомингом
- Информация о управлении питанием моторов (постоянное vs временное)
- Примеры правильного использования GyverPlanner API
- Диагностическая информация и решение частых проблем

### Техническое обоснование
- README теперь соответствует текущей реализации с GyverPlanner
- Учтены все изменения в конфигурации датчиков и моторов
- Добавлена практическая информация для пользователей системы

## [2025-06-25] - Подтверждение получения команды
### Добавлено
- Отправка строки `RECEIVED` при получении любой команды (функция `parseCommand`)

### Изменено
- Функция `parseCommand` теперь выводит подтверждение перед обработкой команды

### Исправлено
- _Нет_

## [2025-06-26] - Переписан homing на planner.setTarget()
### Изменено
- Функция `stepperHome()` полностью переработана и теперь использует `planner.setTarget()` вместо `planner.setSpeed()`.
- Предварительный отъезд, уход при сработавшем датчике, поиск концевика и финальный backoff реализованы через координированные цели.

### Исправлено
- Сообщение `COMPLETE` выводится только после полного завершения движения и обнуления координат.
- Исключены гонки состояния, когда планировщик ещё «занят», а команда уже завершена.

### Добавлено
- Подробное логирование этапов: pre-backoff, move-away, seek, final backoff.
- Предупреждения, если мотор не достиг концевика за `maxSteps`.